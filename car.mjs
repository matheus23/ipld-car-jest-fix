...
var CarIndexedReader = class {
  ...
  async get(key) {
    ...
    if (!this._fd) {
      this._fd = await fs3.promises.open(this._path, "r");
    }
    const readIndex = {
      cid: key,
      length: 0,
  ...
  static async fromFile(path) {
    ...
    const iterable = await CarIndexer.fromIterable(Readable.from(fs3.createReadStream(path)));
    const index = new Map();
    const order = [];
...
export {
  ...
  CarIndexedReader,
  CarIndexer,
  CarReader2 as CarReader,
  CarWriter2 as CarWriter
};
